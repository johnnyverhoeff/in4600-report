...
LEDs can act as beacons for indoor localization, by transmitting information about their location trough visible light communication (VLC).
Since the data they send is unique to their location, it may be possible to identify that same LED when looking at the energy signature with a smart-meter.
To this purpose coding sequences were investigated that would allow for disaggregation of these lights by looking at the aggregated energy uses, so that individual lights can be identified as being on or off.
With piggybacking on VLC, each light will get an unique ID, that they can transmit via VLC for localization purposes and the same ID will also be transmitted through the power they draw.
Hardware was designed, specifically to let that ID map into the energy signature.
And experiments were performed with real consumer LEDs.
...
Furthermore, simulations have been performed in order to show the scalability of the proposed methods.
...
% INTRODUCTION: page 1
\include{chapters/introduction-chapters/introduction}

\include{chapters/related-work-chapters/related-work}

\include{chapters/design-requirements-chapters/design-requirements}

\include{chapters/cdma-chapters/cdma}

\include{chapters/hardware-chapters/hardware-design}

\include{chapters/evaluation-chapters/evaluation}
...
\include{chapters/appendix/dc-test-bed/dc-test-bed-schematic}

\include{chapters/appendix/commercial-230v-ac-modified-schematic/commercial-230v-ac-modified-schematic}

\include{chapters/appendix/custom-modulator/custom-modulator}

\include{chapters/appendix/custom-current-sampler/custom-current-sampler}

\include{chapters/appendix/ac-test-bed/ac-test-bed}
