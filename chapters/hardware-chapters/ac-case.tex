...
The entire modulator's schematic can be found in \todo{Appendixxx}
	...
	The optocoupler makes sure that the output pin is at a safe voltage for the microprocessor.
	...
	\begin{figure}[!tbp]
	  ...
	  \begin{minipage}[b]{0.48\textwidth}
	    ...
	    \caption{Non-disturbing voltage source to power optocouplers.}
	...
	The triggering circuit (\autoref{fig:custom-modulator-trigger}) requires a 5 V power supply in order to drive the optocoupler.
	...
	The capacitor can then provide enough power for the optocoupler until the capacitor can be charged again.
