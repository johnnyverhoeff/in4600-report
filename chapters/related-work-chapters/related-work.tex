...
And finally the proposed method is introduced.
	...
	Since the method is aimed at legacy buildings, we cannot rely an a Building Control Network (BCN).
	...
	But as discussed many lights have the same signature, so it not possible to accurately identify which lights are on using only a smart-meter.
