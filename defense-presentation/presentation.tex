...
%\subsection{Section 1 - Subsection 1}
	...
	\begin{frame}\frametitle{VLC Piggybacking}
		...
		%	\begin{figure}
		%		\centering
		%		\includegraphics[width=\textwidth]{vlc-indoor-positioning-edit2.jpg}
		%	\end{figure}
		%\end{minipage}%
		%\begin{minipage}{.5\linewidth}
		%	\begin{figure}
		%		\centering
		%		\includegraphics[width=0.5\textwidth]{vlc-data.jpg}
		%	\end{figure}
		%\end{minipage}
	...
	% These two terms cause interfence and limit the accuracy of the system, so the cross-correlation must be as close to zero as possible.
	% if they are a large negative number the outcome could result in being a false-negative for example
...
%
	...
	\begin{frame}\frametitle{Mapping Problem}
		...
		\begin{minipage}{.5\linewidth}
			...
			\begin{itemize}
				...
				\item $R_{XY} = -m - 2 \times \hat{R}_{XY}$ %$R_{XY} = f(\hat{R}_{XY})$
			\end{itemize}
	...
	% if the CC is large negative then false negative will occur, because the result sugusts A is off while it is ON.
	% IF we want to see if another light D is on, we know D is off, because it is not transmitting.
	% however if the sums of CC is high positive number  it says that D is on , which is a false positive. 
	...
	% Fast identifyin of all leds will results in a less accurate system, and a more accurate system will result in a greater time needed to identify all the leds
	...
	% for this work a dc and ac hardware is developed, but the dc case is the more trivial one, so in this presentaion we will focus on the AC case.
	...
	% because the triggering circuits are the same here as in the modulator and they both receive the same voltage, both devices are synchronized.
	...
	%this evaluation has run multiple times adn each time similar results were observerd
	...
	%First goal explain:
	% What we set out to do with this work is to see if we could identify which individual lights are on with a single smart-meter.
	%WE could only  measure the aggregated energy signatures and with that we had to identify which lights were on.

	% we have eachiev this by doing the following things

	% Investigatd codes that have resilisnce to each other
	% we made them suitable for the use with LEDs.
	% And come up with a way to identfy a large amount of leds in a timelymanner
	...
	% multiple evulating efforts were made with good results, we were able to identify which leds were on in a timely manner when using DC and AC.
